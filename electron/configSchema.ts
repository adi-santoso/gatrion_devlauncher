...
function migrateConfig(config: unknown, fromVersion?: number): RawConfig {
  if (!config || typeof config !== 'object' || Array.isArray(config)) {
    throw new Error('Config data must be an object')
  }
  if (typeof fromVersion === 'number' && Number.isInteger(fromVersion) && fromVersion > CONFIG_SCHEMA_VERSION) {
    throw new Error(`Unsupported config schema version: ${fromVersion}`)
  }

  const migrated: RawConfig = { ...(config as Record<string, unknown>) }
...
function normalizeConfig(config: unknown = {}): AppConfig {
  const migrated = migrateConfig(config, (config as { schemaVersion?: number } | null)?.schemaVersion)
...
function applyConfigUpdates(current: AppConfig, updates: DeepPartial<AppConfig>): AppConfig {
  ...
  for (const nestedKey of ['notifications', 'terminal', 'autoRestart', 'preview', 'prayer', 'agent', 'windowBounds'] as const) {
  ...
  for (const key of ['sidebarExpanded', 'startOnBoot', 'minimizeToTray', 'autoStartProjects'] as const) {
    if (updates[key] !== undefined && typeof updates[key] !== 'boolean') throw new Error(`${key} must be a boolean`)
  }
  for (const key of ['onStart', 'onError', 'sound'] as const) {
  ...
  for (const adjustmentKey of ['fajr', 'dhuhr', 'asr', 'maghrib', 'isha'] as const) {
...
export { DEFAULT_CONFIG, CONFIG_SCHEMA_VERSION, applyConfigUpdates, normalizeConfig, migrateConfig }
