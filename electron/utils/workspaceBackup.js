 ...
 * @param {{ projects: Array<Record<string, any>>, config: object, presets: Array<object>, health: object, appVersion?: string }} input
 */
function buildBundle({ projects, config, presets, health, appVersion }) {
  return {
    app: 'devlauncher',
    type: BACKUP_TYPE,
    ...
    hasSecrets: Array.isArray(projects)
      && projects.some((project) => (project.envVars || []).some((env) => env.value)),
 ...
 */
function encryptBundle(json, password) {
 ...
 * @param {string} text
 * @param {string} [password]
 * @returns {{ parsed: Record<string, any>, wasEncrypted: boolean }}
 */
function parseBackupFile(text, password) {
 ...
 * @param {Array<Record<string, any>>} currentProjects
 * @param {Array<Record<string, any>>} incoming
 */
function mergeProjects(currentProjects, incoming) {
