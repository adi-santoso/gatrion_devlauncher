...
import type { OmpInstaller, InstallState } from '../managers/OmpInstaller'
import type { OmpConfig } from '../managers/OmpConfig'

const { ipcMain, shell, dialog } = require('electron') as typeof import('electron')
const fs = require('fs')
...
function assertProjectPath(cwd: unknown): string {
...
function setupAgentHandlers(
  ...
  ompManager.on('event', ({ projectId, event }: { projectId: string; event: unknown }) => {
    safeSend('omp-event', { projectId, event })
  })
  installer.on('progress', (state: InstallState) => {
  ...
  secureHandle('omp-update-session-tokens', async (event, projectId, sessionId, tokens, cost) => {
    ...
    const meta: { tokens: number; cost?: number } = { tokens: count }
  ...
  function assertImages(images: unknown): Array<{ type: string; data: string; mimeType: string }> {
    ...
    for (const image of images) {
      const img = image as { type?: unknown; mimeType?: unknown; data?: unknown }
      if (!image || img.type !== 'image') throw new Error('Invalid image attachment')
      if (typeof img.mimeType !== 'string' || !img.mimeType.startsWith('image/')) throw new Error('Invalid image mime type')
      if (typeof img.data !== 'string' || !img.data || img.data.length > MAX_IMAGE_BASE64) throw new Error('Invalid image data')
    }
  ...
  secureHandle('omp-get-models', async (event, projectId, cwd) => {
    ...
    const list = Array.isArray(data) ? data : ((data as { models?: unknown[] } | null)?.models || [])
