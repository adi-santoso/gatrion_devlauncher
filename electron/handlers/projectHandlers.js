...
function setupProjectHandlers(storageManager, processManager, mainWindow) {
  ...
  // Add a project
  handle('add-project', async (event, projectData) => {
  ...
  // Update a project
  handle('update-project', async (event, projectId, updates) => {
  ...
  // Delete a project
  handle('delete-project', async (event, projectId) => {
  ...
  handle('read-env-file', async (event, projectPath, fileName) => {
    ...
    try {
      ...
      if (error && error.code === 'ENOENT') throw new Error(`File ${fileName} does not exist`)
      throw error
    }
