...
import { execNpm, assertSafePackageName } from '../utils/npmRunner'
...
function runGit(cwd: string, args: string[], { timeoutMs = 30000 }: { timeoutMs?: number } = {}): Promise<string> {
...
const cleanPath = (raw: unknown): string => {
...
function parseStatus(output: string) {
  const result: {
    ...
    untracked: string[]
  } = { branch: null, upstream: null, ahead: 0, behind: 0, staged: [], unstaged: [], untracked: [] }
...
function assertBranchName(branch: unknown): string {
...
function assertPathArray(files: unknown): string[] {
 ...
 */
function setupRepoHandlers(storageManager: StorageManager, processManager: ProcessManager, mainWindow: BrowserWindow | null): void {
  ...
  const loadProject = async (projectId: unknown): Promise<Project> => {
  ...
  secureHandle('git-status', async (event, projectPath) => {
    try {
      ...
      if (error && /not a git repository/i.test(err.message)) {
  ...
  function parseStashList(output: string): Array<{ index: number; ref: string; message: string }> {
  ...
  secureHandle('run-project-script', async (event, projectId, scriptName) => {
    try {
      ...
      const result = await processManager.runCustomCommand(
        ...
        (pid: string, log: unknown) => safeSend('process-log', pid, log)
      )
  ...
  secureHandle('install-dependencies', async (event, projectId) => {
    try {
      ...
      return { ...result, success: true, packageManager: pm }
    } catch (error) {
      return { success: false, error: (error as Error).message }
    }
  })
...
export { setupRepoHandlers, parseStatus, parseLog, readPackageJson, runGit }
