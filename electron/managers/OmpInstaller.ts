...
class OmpInstaller extends EventEmitter {
  ...
  async fetchLatestRelease(): Promise<ReleaseInfo> {
    ...
    const exe = (release.assets || []).find((asset: { name?: string }) => asset.name === WIN_ASSET)
    if (!exe) throw new Error(`No ${WIN_ASSET} asset in the latest release`)
    return {
      version: String(release.tag_name || '').replace(/^v/, ''),
      url: exe.browser_download_url,
      size: exe.size,
      checksumsUrl: (release.assets || []).find((asset: { name?: string }) => asset.name === 'SHA256SUMS.txt')?.browser_download_url || null,
    }
  }

  async install(): Promise<InstallState> {
  ...
  _download(url: string, filePath: string, onProgress: (received: number) => void): Promise<void> {
    return new Promise((resolve, reject) => {
      let received = 0
      const file = fs.createWriteStream(filePath)
      const req = https.get(url, { headers: { 'User-Agent': 'Gatrion/1.0' } }, (res: { statusCode?: number; resume(): void; on(event: string, cb: (chunk: Buffer) => void): void; pipe(dest: unknown): unknown }) => {
  ...
  async _verify(filePath: string, checksumsUrl: string | null): Promise<void> {
    const hash = await new Promise<string>((resolve, reject) => {
      const stream = crypto.createHash('sha256')
      const read = fs.createReadStream(filePath)
      read.on('data', (chunk: Buffer) => stream.update(chunk))
  ...
  _get(url: string, headers: Record<string, string> = {}): Promise<string> {
    return new Promise((resolve, reject) => {
      const req = https.get(url, { headers, timeout: 15000 }, (res: NodeJS.ReadableStream) => {
        let data = ''
  ...
  hasManagedBinary(): boolean {
    return require('fs').existsSync(this.targetBinary)
  }
}

export default OmpInstaller


export type { OmpInstaller, InstallState, ReleaseInfo }
