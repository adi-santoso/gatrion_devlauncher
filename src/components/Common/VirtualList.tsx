...
export default function VirtualList<T>({
  ...
  const heightsRef = useRef<Map<string, number>>(new Map()) // item key -> measured px
  const observersRef = useRef<Map<string, ResizeObserver>>(new Map()) // item key -> ResizeObserver
  ...
  const observeRow = (key: string) => (el: HTMLDivElement | null) => {
    if (!el) return
    const update = () => {
