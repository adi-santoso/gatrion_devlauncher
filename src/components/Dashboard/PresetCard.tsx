...
/** Preset enriched with the optional display fields used by the card. */
export interface PresetCardData extends Preset {
  description?: string;
  color?: string;
...
const Chip = ({ count, status }: { count: number; status: string }) => {
...
interface PresetCardProps {
  ...
  progress?: PresetProgress | null;
  onStart?: (preset: Preset) => void;
  onStop?: (preset: Preset) => void;
  onRestart?: (preset: Preset) => void;
  onEdit?: (preset: Preset) => void;
  onDuplicate?: (preset: Preset) => void;
  onDelete?: (preset: Preset) => void;
  onMove?: (presetId: string, direction: number) => void;
}
...
export default function PresetCard({
  ...
  const presetProjects = (preset.projectIds || [])
    ...
    .filter((project): project is ProjectRuntime => Boolean(project));
  const counts = {
