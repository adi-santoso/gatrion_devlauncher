...
import type { MetricsResult } from '../../data/processes';

/**
 * Project as consumed by dashboard cards — persisted fields plus runtime
 * status/metrics and a few legacy aliases still produced by older event
 * payloads (stack, cpuUsage, mem, url, protocol, ...).
 */
export interface ViewProject extends ProjectRuntime {
  stack?: string;
...
const Sparkline = ({ samples, stroke = '#34d399', height = 22 }: SparklineProps) => {
  const points = useMemo(() => {
    const values = (samples || []).filter((value): value is number => value != null && Number.isFinite(value));
...
interface ProjectCardProps {
  ...
  onNavigate?: (project: ProjectRuntime) => void;
  getMetricHistory?: (projectId: string) => Array<{ t: number; cpu: number | null; memory: number | null }>;
}

export default function ProjectCard({ project, onStart, onStop, onRestart, onNavigate, getMetricHistory }: ProjectCardProps) {
  ...
  const hasLogs = (project.logs?.length ?? 0) > 0;
