...
const isListLine = (line: string): boolean => LIST_LINE.test(line.trim());
...
type MarkdownBlock =
  ...
  | { type: 'ul'; items: Array<{ text: string; bullet: string | null }> }
...
export default function Markdown({ content }: { content: string }) {
  ...
  while (i < lines.length) {
    ...
    if (isListLine(trimmed)) {
      const items: Array<{ text: string; bullet: string | null }> = [];
      while (i < lines.length && isListLine(lines[i])) {
        const current = lines[i].trim();
        const marker = current.match(LIST_LINE)?.[0] || '';
        ...
        items.push({ text: current.slice(marker.length), bullet: /^[-*+]$/.test(bullet) ? null : bullet });
        i += 1;
      }
    ...
    <div className="space-y-1.5">
      {blocks.map((block, index) => {
        ...
        switch (block.type) {
          ...
          case 'heading': {
            const Tag: 'h2' | 'h3' | 'h4' | 'h5' = block.level === 1 ? 'h2' : block.level === 2 ? 'h3' : block.level === 3 ? 'h4' : 'h5';
