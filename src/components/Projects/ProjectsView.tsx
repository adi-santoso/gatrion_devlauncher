...
interface ProjectsViewProps {
  ...
  onStart?: (project: ViewProject) => void;
  onStop?: (project: ViewProject) => void;
  onForceStop?: (project: ViewProject) => void;
  onRestart?: (project: ViewProject) => void;
  onDelete?: (project: ViewProject) => void;
  onBulkStart?: (projects: ViewProject[]) => void;
  onBulkStop?: (projects: ViewProject[]) => void;
  onBulkRestart?: (projects: ViewProject[]) => void;
  onBulkDelete?: (projects: ViewProject[]) => void;
  onBulkTagEdit?: (projects: ViewProject[], tagsToAdd: string[], tagsToRemove: string[]) => void;
  onEdit?: (project: ViewProject) => void;
  onNavigate?: (project: ViewProject) => void;
  onOpenModal?: () => void;
  onDuplicate?: (project: ViewProject) => void;
  onBulkSavePreset?: (projectIds: string[]) => void;
}
...
export default function ProjectsView({
  ...
  const sortedProjects = useMemo(() => {
    ...
    const compare = (a: ViewProject, b: ViewProject): number => {
  ...
  const sortIndicator = (key: string): JSX.Element => {
  ...
  useEffect(() => {
    const handler = (event: KeyboardEvent): void => {
      ...
      const isTyping = target
        && (target.tagName === 'INPUT' || target.tagName === 'SELECT' || target.tagName === 'TEXTAREA' || (target as HTMLElement & { isContentEditable?: boolean }).isContentEditable);
  ...
  const statusCounts = useMemo(() => {
    const counts: Record<string, number> = { running: 0, starting: 0, stopping: 0, stopped: 0, error: 0 };
    ...
    <div className="view space-y-4">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)} aria-label="Filter by type" className="bg-surface-3 border border-border rounded-lg px-3 py-2 text-xs text-ink-soft focus:outline-none">
          ...
          {types.map((type) => <option key={type} value={type}>{typeLabel(type as ProjectType)}</option>)}
        </select>
