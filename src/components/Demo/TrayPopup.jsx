 ...
 */
const TrayPopup = ({ isOpen, projects = [], runningProjects, onClose, onStopProject, onQuit }) => {
  if (!isOpen) return null;

  const activeProjects = runningProjects 
    ? runningProjects 
  ...
  return (
    ...
    >
      <p className="px-3 pb-1.5 text-[10px] font-semibold uppercase tracking-wider text-ink-faint">
        Running ({activeProjects.length})
      </p>
      {activeProjects.length === 0 ? (
        <p className="px-3 py-1 text-xs text-ink-faint italic">No projects running</p>
      ) : (
        activeProjects.map((project) => (
          ...
          >
            ...
            <button
              onClick={() => onStopProject && onStopProject(project.id || project)}
              className="text-ink-faint hover:text-danger px-1.5 py-0.5 rounded hover:bg-danger/10"
            >
              Stop
            </button>
          </div>
