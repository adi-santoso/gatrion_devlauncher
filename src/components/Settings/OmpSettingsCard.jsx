...
export default function OmpSettingsCard({ onNotifyStatus }) {
  const [status, setStatus] = useState(null);
  const [statusLoading, setStatusLoading] = useState(true);
  const [config, setConfig] = useState({ providers: [], defaultModel: null });
  const [configLoading, setConfigLoading] = useState(true);
  const [installState, setInstallState] = useState({ status: 'idle' });
  const [updateInfo, setUpdateInfo] = useState(null);
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState(null); // { type: 'success'|'error', text }
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState({ name: '', baseUrl: '', api: 'openai-completions', apiKey: '', models: '', authHeader: false, disableStrictTools: false });
  ...
  useEffect(() => {
    ...
    return ipc.onOmpInstallProgress((state) => setInstallState(state));
  }, [refreshAll]);
  ...
  const handleCheckUpdate = async () => {
    ...
    if (result?.success) {
      ...
      notify('success', result.latest ? `Latest omp release: ${result.latest} (${(result.size / 1048576).toFixed(0)} MB)` : 'Already up to date');
    } else {
  ...
  const handleSaveProvider = async () => {
    ...
    if (result?.success) {
      ...
      setShowForm(false);
      setForm({ name: '', baseUrl: '', api: 'openai-completions', apiKey: '', models: '', authHeader: false, disableStrictTools: false });
      await refreshConfig();
    } else {
  ...
  const handleSetDefault = async (modelRef) => {
    setMessage(null);
      ...
      <div className="flex items-center gap-2.5">
        ...
        {statusLoading ? (
          ...
          <span className="flex items-center gap-1.5 text-[10px] text-success bg-success/10 border border-success/20 rounded-full px-2 py-0.5">
            <StatusDot ok color="success" /> {status.version || 'installed'}
          </span>
        ) : (
      ...
      <div className="space-y-2">
        {!installed ? (
          <>
            ...
            {updateInfo?.latest && <p className="text-[10px] text-ink-faint">Latest release: v{updateInfo.latest} (~{(updateInfo.size / 1048576).toFixed(0)} MB)</p>}
          </>
        ) : (
          <>
            <div className="flex flex-wrap items-center gap-2">
