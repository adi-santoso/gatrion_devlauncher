...
export default function TerminalWorkspace({ projects = [], getLogs, onClearLogs }) {
  ...
  return (
    <div className="view mx-auto max-w-[1400px]">
      <header className="mb-5">
        ...
        <p className="mt-1 text-xs text-ink-soft">Read-only output from active project processes.</p>
      </header>
      <div className="grid min-h-[520px] overflow-hidden rounded-xl border border-border bg-surface shadow-card md:grid-cols-[190px_minmax(0,1fr)]">
        ...
        <div className="min-w-0 p-3">
          <LogsTab logs={logs} onClear={selectedProject ? () => onClearLogs(selectedProject.id) : undefined} />
        </div>
      </div>
    </div>
  );
