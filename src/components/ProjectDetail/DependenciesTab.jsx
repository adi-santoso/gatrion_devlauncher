...
export default function DependenciesTab({ project }) {
  const projectPath = project?.path;
  const [outdated, setOutdated] = useState([]);
  const [loading, setLoading] = useState(true);
  const [notice, setNotice] = useState(null);
  const [busy, setBusy] = useState(null);
  const [confirmUpdate, setConfirmUpdate] = useState(null); // { name, target }
  ...
  const refresh = useCallback(async () => {
    ...
    if (result.success) {
      ...
      if (!result.hasPackageJson) setNotice({ type: 'info', message: 'No package.json found in this project.' });
    } else {
  ...
  const handleUpdate = async (name = null) => {
