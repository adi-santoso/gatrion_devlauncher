...
const EMPTY_STATUS = { isRepo: true, branch: null, upstream: null, ahead: 0, behind: 0, staged: [], unstaged: [], untracked: [] };
...
const CODE_LABEL = { A: 'A', M: 'M', D: 'D', R: 'R', C: 'C', U: 'U', T: 'T', '?': '?' };
...
export default function GitTab({ project }) {
  ...
  const runAction = async (action, label) => {
    ...
    try {
      ...
      setNotice({ type: 'error', message: error.message || `${label} failed` });
      return false;
    } finally {
      setBusy(null);
    }
  ...
  const handleToggle = async (entry, section) => {
  ...
  const handleShowDiff = async (entry, section) => {
