...
import type { Project, ProjectChanges } from '../types/shared'

/** Project as held in renderer state — persisted fields plus runtime-only status. */
export interface ProjectRuntime extends Project {
  status?: string
  pid?: number | null
  uptime?: string | null
...
/** Updates accepted by `updateProject` — persisted changes plus runtime fields. */
export type ProjectUpdateInput = ProjectChanges & {
  status?: string
  pid?: number | null
  uptime?: string | null
  errorMessage?: string | null
...
export const useProjects = () => {
  const [projects, setProjects] = useState<ProjectRuntime[]>([])
  ...
  const refreshProjects = useCallback(async () => {
    try {
      ...
      if (response.success) {
        ...
        const projectsWithResetStatus: ProjectRuntime[] = (response.projects || []).map(project => ({
          ...project,
          status: 'stopped',
        }))
  ...
  // Add a new project
  const addProject = useCallback(async (projectData: ProjectChanges): Promise<ProjectActionResult> => {
    try {
      const response = await ipc.addProject(projectData)

      if (response.success) {
        setProjects(prev => upsertProject(prev, { ...response.project, status: 'stopped' } as Project))
  ...
  const updateProject = useCallback(async (projectId: string, updates: ProjectUpdateInput): Promise<ProjectActionResult> => {
    try {
  ...
  const updateProjectLocal = useCallback((projectId: string, updates: Record<string, unknown>) => {
    setProjects(prev => prev.map(p =>
      p.id === projectId ? { ...p, ...updates } : p
    ))
  }, [])
  ...
  // Detect project type
  const detectProjectType = useCallback(async (projectPath: string) => {
    try {
  ...
  useEffect(() => {
    const cleanup = ipc.onProjectsUpdated((updatedProjects) => {
      setProjects(prev => {
        ...
        const statusMap: Record<string, Partial<ProjectRuntime>> = {}
        prev.forEach(p => {
          statusMap[p.id] = {
            status: p.status,
            pid: p.pid,
